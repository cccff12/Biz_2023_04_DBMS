...
-- 삭제, 제거 DELETE, DROP
-- DELETE : 데이터의 일부 요소를 삭제하기, 데이터의 구조 등은 변화 없음
-- DROP : 물리적인 데이터 요소를 제거하는, 데이터 구조 제거, 많은 데이터를 한꺼번에 제거
--         데이터 파괴자 명령, 매우 신중하게 주의하면서 사용해야 한다
--DROP TABLESPACE myDB
--INCLUDING CONTENTS AND DATAFILES;

-- 21C에서 일부 권한이 필요한 스크립트 실행가능하도록 설정하기
ALTER SESSION SET "_ORACLE_SCRIPT" = TRUE;

-- 2. DB에 연결하여 실습할 일반 사용자 계정을 생성하기
...
-- 실습의 편리를 위해 SYSDBA보다 한 단계 낮은 일반 DBA권한을 부여
--오라클에서는 SYSDBA(SYSREM 등)의 User 권한은 매우 조심해서 신중하게 다루어야 한다
-- DB 사용의 편의성을 위하여 DBA 권한을 일반 사용자에게 부여할수 있도록 하고있다.

-- $$이 권한은 실제환경 에서는 절대 사용하면 안된다.
-- 시스템의 중요한 데이터 사전을 제외한 모든 데이터를 
--핸들링(추가, 수정, 삭제, 제거 등)을 할수 있기 때문이다
-- 다만 실습에서만 사용한다.
